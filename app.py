...
BUILD_LABEL = 'EMERGENCY DAILY PLAYER V1.5 — DECISION LAYER AUDIT / 6 LANES KEPT'

st.set_page_config(page_title='Emergency Daily Player V1.5', layout='wide')
...
st.title('Emergency Daily Player V1.5')
...
if res:
    ...
    with tabs[2]:
        ...
        if p.exists():
            st.download_button('Download 07_DECISION_LAYER_STRATEGY_SUMMARY.csv', p.read_bytes(), file_name='07_DECISION_LAYER_STRATEGY_SUMMARY.csv', mime='text/csv')
    with tabs[3]:
    ...
    with tabs[7]:
        ...
        if zip_path.exists():
            st.download_button('Download full daily output ZIP', zip_path.read_bytes(), file_name=zip_path.name, mime='application/zip')
        for name, mime in [
            ...
            p = outdir / name
            if p.exists():
                st.download_button(f'Download {name}', p.read_bytes(), file_name=name, mime=mime)
